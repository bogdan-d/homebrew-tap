...
tap_path() {
  brew --repo "$TAP_NAME" 2>/dev/null
}

install_receipt() {
  printf '%s/.metadata/INSTALL_RECEIPT.json\n' "$(brew --caskroom "$TAP_CASK_NAME")"
}
...
load_owned_tap() {
  ...
  TAP_PATH="$(tap_path)" || fail "Scratch tap '${TAP_NAME}' does not exist."
...
cleanup_owned_tap() {
  ...
  then
    ...
    actual_receipt="$(receipt_id "$receipt" 2>/dev/null || true)"
...
case "$COMMAND" in
  install)
    ...
    receipt_id "$receipt" >"$TAP_PATH/$INSTALL_MARKER"
    ;;
  audit)
    BREW_ARGS=(--cask)
    [[ "$VERBOSE" == "false" ]] || BREW_ARGS+=(--verbose)
